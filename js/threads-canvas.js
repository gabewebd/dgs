...
// Initialize OGL WebGL Threads Background
...
if (canvas) {
  ...
  float lineFn(vec2 st, float width, float perc, float offset, vec2 mouse, float time, float amplitude, float distance) {
      ...
      float xnoise = mix(
          ...
          Perlin2D(vec2(time_scaled, st.x + time_scaled) * 3.0) / 1.5,
          st.x * 0.3
      );
  ...
  void mainImage(out vec4 fragColor, in vec2 fragCoord) {
      ...
      fragColor = vec4(uColor * colorVal, colorVal * 0.35); // Lower alpha for subtle background presence
  }
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      iTime: { value: 0 },
      iResolution: { value: new Color(canvas.width, canvas.height, canvas.width / canvas.height) },
      uColor: { value: new Color(0.031, 0.615, 0.529) }, // Customized Brand Teal Color (rgb(8, 157, 135))
      uAmplitude: { value: 0.8 },
      uDistance: { value: 0.15 },
